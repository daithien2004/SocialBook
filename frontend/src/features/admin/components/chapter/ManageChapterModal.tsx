...
export default function ManageChapterModal() {
    ...
    const [paragraphs, setParagraphs] = useState<Paragraph[]>([{ id: crypto.randomUUID(), content: '' }]);
            ...
            if (isManageChapterOpen && manageChapterData?.chapter) {
                ...
                try {
                    ...
                    setParagraphs(fullChapter.paragraphs && fullChapter.paragraphs.length > 0 
                        ? fullChapter.paragraphs.map((p: Paragraph) => ({ ...p })) 
                ...
                setTitle('');
                setParagraphs([{ id: crypto.randomUUID(), content: '' }]);
            }
        };

        loadChapterDetails();
    }, [isManageChapterOpen, manageChapterData, closeManageChapter]);
    ...
    const addParagraph = () => {
        setParagraphs([...paragraphs, { id: crypto.randomUUID(), content: '' }]);
