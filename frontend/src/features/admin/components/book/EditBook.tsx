...
import { useUpdateBook } from '@/features/books/api/books.mutations';
...
import type { Author, Genre, BookForAdmin } from '@/features/books/types/book.interface';
...
export default function EditBook({ bookId }: EditBookProps) {
    ...
    const { data: bookData, isLoading: loadingBook, error: bookError } = useQuery(bookQueries.byId(bookId));
    const { data: authors = EMPTY_AUTHORS, isLoading: loadingAuthors } = useQuery(authorQueries.all());
    const { data: genres = EMPTY_GENRES, isLoading: loadingGenres } = useQuery(genreQueries.all());
...
function EditBookForm({ bookId, bookData, authors, genres, loadingAuthors, loadingGenres }: EditBookFormProps) {
    const router = useRouter();
    ...
    const [formData, setFormData] = useState<FormData>(() => ({
        ...
        status: (bookData.status as Status) || 'draft',
        tagsInput: bookData.tags?.join(', ') || '',
    }));

    const [coverPreview, setCoverPreview] = useState<string>(bookData.coverUrl || DEFAULT_COVER);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            await updateBook.mutateAsync({ bookId, formData: formPayload });
