...
import { useCreateBook } from '@/features/books/api/books.mutations';
...
export default function CreateBook() {
  ...
  const { data: authorsData = EMPTY_AUTHORS, isLoading: loadingAuthors } = useQuery(authorQueries.all());
  const { data: genresData = EMPTY_GENRES, isLoading: loadingGenres } = useQuery(genreQueries.all());
  const { data: filtersData } = useQuery(bookQueries.filters());
  ...
  const {
    ...
    handleReset,
    handleSubmit,
  } = useCreateBookForm((payload) => createBook.mutateAsync(payload));
