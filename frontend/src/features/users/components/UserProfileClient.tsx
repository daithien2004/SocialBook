...
import { useUpdateUserOverview, useUpdateUserAvatar } from '@/features/users/api/users.mutations';
...
export function UserProfileClient({ userId }: { userId: string }) {
    const { mutateAsync: updateOverview } = useUpdateUserOverview();
    const { mutateAsync: updateAvatar } = useUpdateUserAvatar();

    const { data: overview } =
        useQuery({
            ...userQueries.overview(userId),
            enabled: !!userId,
        });
    ...
    return (
        <Card className="border-border shadow-sm">
          <CardContent className="p-0">
            {/* Header */}
            <div className="relative rounded-t-2xl bg-muted px-6 py-10 md:py-12">
