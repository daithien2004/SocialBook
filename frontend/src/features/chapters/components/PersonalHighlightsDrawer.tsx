...
const HighlightItem = ({ highlight, onSelect }: { highlight: UserHighlight; onSelect?: (h: UserHighlight) => void }) => {
  ...
  const handleSaveNote = async () => {
    await updateHighlight.mutateAsync({ id: highlight.id, note: noteContent });
    setIsEditingNote(false);
  };

  const handleColorChange = (color: string) => {
    updateHighlight.mutate({ id: highlight.id, color });
  };
    ...
    <div className="flex flex-col gap-3 p-4 rounded-xl border border-border/50 bg-muted/30 relative group">
      ...
      <div className="flex items-center justify-between text-[11px] text-muted-foreground">
        ...
        <div className="flex items-center gap-1">
          ...
          <button 
            onClick={(e) => { e.stopPropagation(); deleteHighlight.mutate(highlight.id); }}
      ...
      {isEditingNote ? (
        <div className="space-y-2 mt-2" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex gap-2 justify-end">
            <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={(e: React.MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); setIsEditingNote(false); }}>
              Hủy
            </Button>
            <Button size="sm" className="h-7 text-xs px-4" onClick={(e: React.MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); handleSaveNote(); }}>
              Lưu ghi chú
            </Button>
          </div>
        </div>
...
export const PersonalHighlightsDrawer = ({ open, onOpenChange, bookId, bookSlug: propBookSlug, currentChapterId, chapters }: PersonalHighlightsDrawerProps) => {
  ...
  const { data: highlights = [], isLoading } = useQuery({
    ...userHighlightQueries.byBook(bookId),
    enabled: !!open && !!bookId
  });
