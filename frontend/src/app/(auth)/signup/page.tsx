...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupFormValues) => {
    try {
      ...
      // Thành công → chuyển thẳng sang verify OTP
      router.push(`/verify-otp?email=${encodeURIComponent(data.email)}`);
    } catch (err: any) {
      // Xử lý lỗi (RTK Query sẽ tự hiện ở {error && ...} bên dưới)
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 space-y-6 transform transition-all duration-300 hover:shadow-3xl">
          <div className="text-center">
            <h2 className="text-3xl font-serif font-bold text-indigo-900">Tạo Tài Khoản</h2>
            <p className="text-sm text-gray-500 mt-2">Tham gia cộng đồng yêu sách của chúng tôi</p>
          </div>
          ...
          >
            ...
            <div className="relative">
              ...
              />
              {formErrors.username && (
                <p id="username-error" className="text-sm text-red-500 mt-1 animate-fade-in">
