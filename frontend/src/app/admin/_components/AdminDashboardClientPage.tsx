...
const UserGrowthChart = dynamic(
  () => import('@/features/admin/components/dashboard/UserGrowthChart').then((mod) => mod.UserGrowthChart),
...
const GenreDistributionChart = dynamic(
  () => import('@/features/admin/components/dashboard/GenreDistributionChart').then((mod) => mod.GenreDistributionChart),
  { ssr: false, loading: () => <div className="h-[300px] rounded-xl flex justify-center items-center"><LoadingSpinner /></div> }
);
...
export default function AdminDashboardClientPage() {
  ...
  const isReindexing = reindexAll.isPending;
  ...
  const handleReindex = async () => {
    try {
      await reindexAll.mutateAsync(undefined);
