...
import { Popconfirm, message, Button } from 'antd'; // ← Thêm Antd
...
export default function AdminAuthorsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    try {
      ...
      message.success(`Đã xóa tác giả "${name}" thành công`);
      refetch(); // Cập nhật lại danh sách ngay lập tức
    } catch (error) {
      message.error('Xóa tác giả thất bại. Vui lòng thử lại.');
                      ...
                      <tr key={author.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex justify-center gap-2">
                            ...
                            {/* Nút Xóa với Popconfirm đẹp như trang Users */}
                            <Popconfirm
