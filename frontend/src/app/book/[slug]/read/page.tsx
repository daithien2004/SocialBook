...
import { BOOKS_ENDPOINTS } from '@/src/app/api/constants';
import {
  ChevronLeft,
  ChevronRight,
  Settings,
...
export default function ReadingPage() {
  ...
  const [preloadedNextChapter, setPreloadedNextChapter] =
    useState<NextChapterData | null>(null);
  const [preloadedNextContent, setPreloadedNextContent] = useState<
    string | null
  >(null);
  ...
  const fetchBookData = async (bookSlug: string) => {
    try {
      const response = await fetch(
        BOOKS_ENDPOINTS.bookAndFirstChapter(bookSlug)
      );
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        const targetChapter =
          data.allIndex.find((ch) => ch.orderIndex === currentChapterIndex) ||
          data.firstChapter;
  ...
  return (
    <div className="min-h-screen bg-background">
      <div
        className={`min-h-screen mt-12 transition-colors duration-300 ${
          isDarkMode
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                      {bookData.allIndex.map((chapter) => (
                        <Link
                          ...
                          className={`block p-2 rounded text-sm hover:bg-accent transition-colors ${
