...
import { BadRequestDomainException, NotFoundDomainException, InternalServerDomainException } from '@/shared/domain/common-exceptions';
...
export class GenerateChapterAudioUseCase {
    ...
    async execute(chapterIdStr: string, options: GenerateAudioOptions = {}): Promise<any> {
        ...
        // 5. Check Cache
        if (!forceRegenerate) {
            const existing = await this.ttsRepository.findExisting(chapterId.toString(), language, voice);
            if (existing) {
                // Sync ttsStatus back to chapter document (may have been missing before)
                await this.chapterRepository.updateTtsStatus(chapterId.toString(), 'completed', existing.audioUrl);
                return existing;
            }
        }
        ...
        try {
            ...
            await this.chapterRepository.updateTtsStatus(chapterId.toString(), 'processing');
            ...
            await this.chapterRepository.updateTtsStatus(chapterId.toString(), 'completed', audioUrl);
            ...
            await this.ttsRepository.save(savedTTS);
            await this.chapterRepository.updateTtsStatus(chapterId.toString(), 'failed');
            throw new InternalServerDomainException(`Failed to generate audio: ${error.message}`);
        }
    }
}
