    ...
    try {
      ...
      if (isSemantic) {
        ...
        books.forEach((b, index) =>
          bookScoreMap.set(b.id.toString(), {
  ...
  private async semanticSearch(
    ...
    try {
      ...
      return results
        .filter(
          (r) =>
            r.document.metadata.type === 'book' ||
            r.document.contentType.toString() === 'book',
        )
        .map((r) => ({
  ...
  private sortBooks(
    ...
    return [...books].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'likes':
          return (a.likes - b.likes) * mul;
        case 'rating':
          return (a.stats.rating - b.stats.rating) * mul;
        case 'score':
