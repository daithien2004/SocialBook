...
export class UnlockAchievementUseCase {
  ...
  async execute(command: UnlockAchievementCommand): Promise<UserAchievement> {
    try {
      ...
      let userAchievement =
        await this.userAchievementRepository.findByUserAndAchievement(
          userId,
          achievementId,
        );
      ...
      this.logger.log(
        `Achievement ${command.achievementId} progress updated for user ${command.userId}`,
      );

      return userAchievement;
    } catch (error) {
      this.logger.error(
        `Failed to unlock achievement ${command.achievementId} for user ${command.userId}`,
        error,
      );
      throw error;
    }
