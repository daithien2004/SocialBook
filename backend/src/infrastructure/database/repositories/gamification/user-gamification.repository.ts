...
import {
  ...
  UserGamificationFilter,
} from '@/domain/gamification/repositories/user-gamification.repository.interface';
  ...
  implements IUserGamificationRepository
{
  ...
  async findById(
    ...
    const document = await this.userGamificationModel
  ...
  async findAll(
  ...
  ): Promise<PaginatedResult<UserGamificationEntity>> {
    ...
    if (filter.maxStreak !== undefined) {
      queryFilter.currentStreak = {
        ...queryFilter.currentStreak,
        $lte: filter.maxStreak,
      };
    }
  ...
  async getTopUsersByStreak(limit: number): Promise<UserGamificationEntity[]> {
    const documents = await this.userGamificationModel
      .find()
  ...
  async getActiveStreaksCount(): Promise<number> {
    return await this.userGamificationModel
      .countDocuments({
        currentStreak: { $gt: 0 },
      })
      .exec();
  ...
  async existsByUser(userId: UserId): Promise<boolean> {
    const count = await this.userGamificationModel
      .countDocuments({
        userId: new Types.ObjectId(userId.toString()),
      })
      .exec();
  ...
  ): Partial<UserGamificationDocument> {
