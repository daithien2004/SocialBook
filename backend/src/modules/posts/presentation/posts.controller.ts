...
export class PostsController {
  ...
  async findAll(@Query() query: PaginationDto) {
    ...
    return {
      ...
      data: PostModal.fromArray(result.data), // Ensure PostModal can handle Post Entity (mapped with needed props)
      meta: {
        page: query.page,
        limit,
        total: result.total,
  ...
  async findAllByUser(@Req() req: Request & { user?: { id: string } }, @Query() query: PaginationUserDto) {
    ...
    return {
      message: 'Get posts successfully',
      data: PostModal.fromArray(result.data),
      meta: {
        page: query.page,
        limit,
        total: result.total,
  ...
  async create(
    ...
    // Check if flagged to add warning message
    const modal = new PostModal(data);
    if (data.isFlagged) {
         return {
            ...modal,
