...
export class VectorDocument extends Entity<VectorId> {
  ...
  getWordCount(): number {
    return this._props.content.split(/\s+/).filter((word) => word.length > 0)
      .length;
  }
