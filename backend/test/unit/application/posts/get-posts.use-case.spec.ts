...
describe('GetPostsUseCase (Unit)', () => {
    ...
    it('page=1, limit=10 → skip=0', async () => {
        ...
        expect(mockPostRepository.findAll).toHaveBeenCalledWith({ skip: 0, limit: 10 });
    });
    ...
    it('page=2, limit=10 → skip=10', async () => {
        ...
        expect(mockPostRepository.findAll).toHaveBeenCalledWith({ skip: 10, limit: 10 });
    });

    it('should propagate repository errors', async () => {
        mockPostRepository.findAll.mockRejectedValue(new Error('DB connection failed'));

        await expect(useCase.execute(new GetPostsQuery(1, 10)))
